...
class PropertyFolder(db.Model):
    __tablename__ = 'bp_portfolio_file'
    
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    user_id = db.Column(db.Integer, db.ForeignKey('bp_users.bp_user_id'), nullable=False)
    name = db.Column(db.String(255), nullable=False)
    description = db.Column(db.Text, nullable=True)
    config_city = db.Column(db.String(50), nullable=False, default='mexico')  # City configuration
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # Relationships
    user = db.relationship('BPUsers', backref='portfolio_files')
    folder_properties = db.relationship('FolderProperty', backref='portfolio_file', cascade='all, delete-orphan')
    
    __table_args__ = (
        db.UniqueConstraint('user_id', 'name', 'config_city', name='unique_user_folder_name_city'),
    )
...
class FolderProperty(db.Model):
    __tablename__ = 'bp_file_map'
    
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    folder_id = db.Column(db.Integer, db.ForeignKey('bp_portfolio_file.id'), nullable=False)
