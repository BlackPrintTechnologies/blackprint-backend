...
where fid = {fid}
    ...
    def get_property_with_market_data_query(filter, city='mexico', show_all_keys=True):
        """
        Get property data with market data joined for Mexico City
        This combines property information with market data from dim_market_data_combined
        """
        if city == 'queretaro' or city == 'el_marques':
            # For QRO, use the existing logic with include_market_join=True
            return QueryController.get_property_query(filter, city, show_all_keys, include_market_join=True)
        else:
            ...
            query += f'''
                ...
                LEFT JOIN blackprint_db_prd.presentation.dim_market_data_combined mdc 
                    ON (
                        (p.ids_market_data_spot2 = mdc.id_market_data AND mdc.source = 'spot2')
                        OR (p.ids_market_data_inmuebles24 = mdc.id_market_data AND mdc.source = 'inmuebles24')
                        OR (p.ids_market_data_propiedades = mdc.id_market_data AND mdc.source = 'propiedades')
                    )
